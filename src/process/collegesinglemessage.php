...
$row = mysql_fetch_array($result);
if($row['college']=='' || $row['id']==null){
	$accept=false;
	$messagereturn .= 'The college could not be found<br />';
}
echo '{';
if($accept==true){//If the message can be processed:
	mysql_query("INSERT INTO collegetalk
		(collegeid, userid, text)
		VALUES ('$collegeid','$userid','$messagetext')");
	echo '"accept":true';
}else{
	echo '"accept":false,';
